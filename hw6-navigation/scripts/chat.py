...
class Chat:
	...
	self.soundhandle.say('Hello,What can I do for you?')
        ...
        self.navi = rospy.Publisher("/nav", String, queue_size=10) 

        # Subscribe to the navigation result
        #self.navigation_back=""
        #rospy.Subscriber('/nav_find_point', String, self.naviback)
	...
	if msg.data.find('DAY')>-1:
        	...
        	self.soundhandle.say("Hello, my friend. Let's have a chat.", volume=0.1)
                ...
                self.soundhandle.say("I am Jack.Nice to meet you.", volume=0.1)
        elif msg.data.find('LIVINGROOM')>-1:
             rospy.loginfo("I am going to the livingoom.")
             self.soundhandle.say("I am going to the livingoom.", volume=0.2)
             self.navi.publish('livingroom')
             ...
             self.navi.publish('kitchen')
	elif msg.data.find('SOFA')>-1:
             rospy.loginfo("I am going to the sofa.")
             self.soundhandle.say("I am going to the sofa.", volume=0.2)
             self.navi.publish('sofa')
